 ...
 */
public class InsertNewNote extends AsyncTask<String, Void, String> {

    private AsyncTaskCompleteListener callback;
    private Context context;
    private String noteId, title, content, date, type, essayId, studentId, file, action;

    //default constructor
    public InsertNewNote(Context context, AsyncTaskCompleteListener cb) {
        //set the context
    ...
    protected String doInBackground(String... params) {
        ...
        String login_url = "http://ec2-52-50-204-225.eu-west-1.compute.amazonaws.com/NoteInsert.php";
        ...
        try {
            ...
            String data2 = URLEncoder.encode("content", "UTF-8") + "=" + URLEncoder.encode(content, "UTF-8");
            String data3 = URLEncoder.encode("date", "UTF-8") + "=" + URLEncoder.encode(date, "UTF-8");
            String data4 = URLEncoder.encode("type", "UTF-8") + "=" + URLEncoder.encode(type, "UTF-8");
            String data5 = URLEncoder.encode("essay", "UTF-8") + "=" + URLEncoder.encode(essayId, "UTF-8");
            String data6 = URLEncoder.encode("student", "UTF-8") + "=" + URLEncoder.encode(studentId, "UTF-8");
            String data7 = URLEncoder.encode("id", "UTF-8") + "=" + URLEncoder.encode(noteId, "UTF-8");
            String data8 = URLEncoder.encode("file", "UTF-8") + "=" + URLEncoder.encode(file, "UTF-8");
            String data9 = URLEncoder.encode("action", "UTF-8") + "=" + URLEncoder.encode(action, "UTF-8");
            //write the encoded data
            bufferedWriter.write(data1 + "&" + data2 + "&" + data3 + "&" + data4 + "&" + data5 + "&" + data6 + "&" + data7 + "&" + data8 + "&" + data9);
    ...
    protected void onPostExecute(String result) {
        ...
        if (result.equals("done")) { //if result is "false" show a toast saying login has failed
            if (action != "edit"){
                Toast.makeText(MainActivity.context, "New Note Added", Toast.LENGTH_LONG).show();
            }
